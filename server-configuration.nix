...

{
  ...
  security.acme = {
    ...
    certs."{{web-domain}}" = {
      ...
      environmentFile = "/etc/nixos/namecheap.env"; # -rw------- 1 root root
  ...
  services.nginx = {
    ...
    appendHttpConfig = ''
      ...
      server {
        ...
        location /miniflux/ {
          # See: https://miniflux.app/docs/howto.html#reverse-proxy-subfolder
          proxy_pass http://127.0.0.1:{{miniflux-port}}/miniflux/;
          proxy_redirect off;
  ...
  services.miniflux = {
    enable = true;
    adminCredentialsFile = "/etc/nixos/miniflux.env"; # -rw------- 1 root root
    # The above config file contains two lines:
    #   ADMIN_USERNAME=admin
    #   ADMIN_PASSWORD=<random numbers and letters>
    config = {
      # https://miniflux.app/docs/configuration.html
      BASE_URL = "https://{{web-domain}}:4430/miniflux/";
      LISTEN_ADDR = "127.0.0.1:{{miniflux-port}}";
    };
  };
  # Miniflux turns on postgresql, which sometimes requires you manually specify a major version.
  # If you don't, then you may get an error that says something like:
  #   error: postgresql_9_6 was removed, please upgrade your postgresql version.
  # That version number comes from the stateVersion in this file looked up in a else-if chain here:
  # https://github.com/NixOS/nixpkgs/blob/49b9166ece0fde26375a59b06528c57371e28642/nixos/modules/services/databases/postgresql.nix#L553-L567
